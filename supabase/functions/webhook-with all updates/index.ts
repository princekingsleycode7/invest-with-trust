...
serve(async (req) => {
  ...
  try {
    ...
    if (payload.event === "charge.success") {
      ...
      // Update user's total investment in their profile
      ...
      if (profileUpdateError) {
        console.error("❌ Failed to update user profile total_invested:", profileUpdateError);
        // Note: Investment is already marked active, but profile wasn't updated
      } else {
        console.log(`📊 User ${investment.user_id} total_invested updated by ${amount}`);
      }
